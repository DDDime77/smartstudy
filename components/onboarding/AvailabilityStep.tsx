...
export default function AvailabilityStep({ value, onChange }: AvailabilityStepProps) {
  ...
  const applyPreset = (presetName: string) => {
    ...
    switch (presetName) {
      ...
      case 'evenings':
        // All days, 6-10 PM (evening activities)
      ...
      case 'work_hours':
        // Mon-Fri, 8 AM - 6 PM (typical work hours)
        newAvailability = [0, 1, 2, 3, 4].map((day) => ({
          day,
          slots: [
            { start: '08:00', end: '10:00' },
  ...
  return (
    <div className="space-y-8 animate-slide-up">
      <div className="text-center mb-8">
        ...
        <h3 className="text-xl font-semibold text-white mb-2">Set Your Busy Schedule</h3>
        <p className="text-slate-400 text-sm">
          Select the times when you're busy (school, work, extracurriculars, etc.)
        </p>
      </div>
      ...
      {totalHours > 0 && (
        <div className="p-4 rounded-lg bg-white/5 border border-white/10 text-white text-sm flex items-center justify-between animate-fade-in">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-medium">Total Weekly Busy Time</div>
              <div className="text-xs text-slate-400 mt-0.5">
                {totalHours} hours busy per week
              </div>
            </div>
          </div>
      ...
      {totalHours === 0 && (
        <div className="p-4 rounded-lg bg-white/5 border border-white/10 text-slate-400 text-sm flex items-start gap-3">
          ...
          <div>
            <div className="font-medium text-white">No busy times selected yet</div>
            <div className="text-xs mt-1">
              Select when you're busy (school, work, etc.) so we can schedule study time around it
            </div>
          </div>
        </div>
      )}
